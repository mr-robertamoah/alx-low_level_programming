...
)
{
	...
	if (h == lastnext && h->next && h->next->next == lastnext)
	{
		free(h->next);
		free(h);
		*nodes = 2;
	...
	if (h->next != NULL)
		free_node(h->next, lastnext, nodes, count, counth);
...
size_t free_list(listint_t **h, listint_t *lastnext)
{
	...
	free_node(*h, lastnext, &nodes, &count, &counth);
